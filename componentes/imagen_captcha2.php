...
$captcha = imagecreatetruecolor(151,40);
//$color = rand(125,160);
$background_color = imagecolorallocate($captcha, 179, 217, 255);
imagefill($captcha, 0, 0, $background_color);

#dibujar algunas lineas
for($i=0;$i<10;$i++){
    $color = rand(48,96);
...
#pasar a mayusculas y reemplazar "O" y "0" para evitar errores
$string = strtoupper($string);
$string = str_replace("O","B", $string);
$string = str_replace("0","C", $string);

#guardar en la sesion "captcha"
session_start();
$_SESSION["captcha"]=$string;

#colocar cada caracter en una posicion aleatoria
$font = 'arial.ttf';
for($i=0;$i<5;$i++){
    $color = rand(0,32);
    if(file_exists($font)){
...
#aplicar distorsion a la imagen
